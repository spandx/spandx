...
RSpec.describe Spandx::Js::Parsers::Yarn do
  ...
  describe '#parse small lock file' do
    ...
    it 'parses small lock file' do
      ...
      expect(res.first.name).to eq('babel')
      expect(res.first.version).to eq('6.23.0')
    end
  end
  ...
  describe '#invalid lock file' do
    ...
    it 'returns nil for invalid version' do
      expect(subject.parse(lockfile)).to eq(nil)
    end
  end
  ...
  describe '#parse long lock file' do
    ...
    it 'parses long yarn.lock file' do
      res = subject.parse(lockfile).to_a
      expected_dependencies = ['accepts@1.3.7', 'array-flatten@1.1.1', 'body-parser@1.19.0', 'bytes@3.1.0', 'content-disposition@0.5.3', 'content-type@1.0.4',
      ...
      expect(res.map { |x| "#{x.name}@#{x.version}" }) .to match_array(expected_dependencies)
