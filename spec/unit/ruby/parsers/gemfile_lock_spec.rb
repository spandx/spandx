...
RSpec.describe Spandx::Ruby::Parsers::GemfileLock do
  ...
  describe '#parse' do
    ...
    context 'when parsing a Gemfile.lock with multiple dependencies' do
      ...
      specify { expect(spandx.meta[:dependencies].map(&:name)).to match_array(%w[addressable bundler nanospinner net-hippie nokogiri parslet terminal-table thor tty-screen zeitwerk]) }
